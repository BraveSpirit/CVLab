...
class Sample4View extends SampleViewBase {
    ...
    private Mat mYuv;
    private Mat mRgba;
    private Mat mRgbaMatches = new Mat();
    private Mat mRgb;
    private Mat mGraySubmat;
    private Mat mGraySubmatOld;
	...
	protected void onPreviewStarted(int previewWidtd, int previewHeight) {
        ...
        mBitmapMatched = Bitmap.createBitmap(previewWidtd*2, previewHeight, Bitmap.Config.ARGB_8888);
    ...
    protected Bitmap processFrame(byte[] data) {
        ...
        case VIEW_MODE_FEATURES_KLT:
        	try{
	            ...
	            mKeyPts.fromArray(returnedKeyPts);
				     ...
				     */
		        if(mRgbOld != null){
		        	mRgbaMatches = new Mat(new Size(2*mRgbaOld.cols()+mRgba.cols(), 2*mRgba.rows()),mRgba.type());
					...
					AndroidOutput ao = new AndroidOutput();
					ao.printInLogMat(mPtsToDraw, "Matched KeyPts");
				    ...
				    if(!mPtsToDraw.empty()){
				    	...
				    	Scalar mColor = new Scalar(0.0,1.0,0.0);
				    	MatOfByte mOB  = new MatOfByte();
				    	//Features2d.drawMatches(mRgbaOld, mKeyPtsOld, mRgba, mKeyPts, mMatches, mRgbaMatches, Scalar.all(-1), Scalar.all(-1), mOB, 1);
				    	
				    	Features2d.drawMatches(mGraySubmatOld, mKeyPtsOld, mGraySubmat, mKeyPts, mMatches, mRgbaMatches);
				    	//Imgproc.cvtColor(mRgbaMatches, mRgbaMatches, Imgproc.COLOR_RGB2RGBA);
				    }
	        	}
	        	else
	        	{
        ...
        try {
        	if(!mRgbaMatches.empty())
        	{
        		bmp = mBitmapMatched;   		
        		Utils.matToBitmap(mRgbaMatches, bmp);
        		bmp = Bitmap.createScaledBitmap(bmp, mBitmap.getWidth(), mBitmap.getHeight(), true);
        	}
        	else
    ...
    //public native void GetFinal(long addrRgba, long addrRgbaOld, long addrRgbaMatched);
